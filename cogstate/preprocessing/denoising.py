...
try:
    import pywt
except ImportError:  # pragma: no cover - exercised only in lean deployments
...
class WaveletDenoisingConfig:
    ...
    def __post_init__(self) -> None:
        if pywt is None:
            raise ImportError(
                "Wavelet denoising requires the optional PyWavelets dependency"
            )
        try:
            pywt.Wavelet(self.wavelet)
        except ValueError as exc:
            raise ValueError(f"Unknown wavelet: {self.wavelet!r}") from exc
        if self.level is not None and (int(self.level) != self.level or self.level < 1):
        ...
        if not np.isfinite(self.threshold_scale) or self.threshold_scale <= 0:
...
def wavelet_denoise(
    signal: object,
    config: WaveletDenoisingConfig | None = None,
) -> tuple[np.ndarray, WaveletDenoisingReport]:
    """Denoise each channel with DWT detail-coefficient shrinkage."""
    config = config or WaveletDenoisingConfig()
    if pywt is None:  # defensive for type checkers and monkeypatched environments
        raise ImportError("Wavelet denoising requires PyWavelets")
    values = np.asarray(signal, dtype=float)
