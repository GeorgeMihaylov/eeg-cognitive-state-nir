...
class OfflinePreprocessingConfig:
    """Explicit offline configuration with an identity transform by default.

    Every signal-changing stage is opt-in because its benefit is data-dependent
    and it can suppress useful EEG components or alter an established protocol.
    """
    ...
    detect_and_interpolate_bad_channels: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        if self.detrend_order is not None and (
            int(self.detrend_order) != self.detrend_order or self.detrend_order < 0
        ):
            raise ValueError("detrend_order must be a non-negative integer or None")
        if self.filter_mode not in {"zero_phase", "causal"}:
            raise ValueError("filter_mode must be 'zero_phase' or 'causal'")
        if self.reference_method not in {
            "none", "common_average", "median", "robust_average"
        }:
            raise ValueError("Unknown reference_method")
        if (
            not np.isfinite(self.robust_reference_z_threshold)
            or self.robust_reference_z_threshold <= 0
            or int(self.robust_reference_max_iterations)
            != self.robust_reference_max_iterations
            or self.robust_reference_max_iterations < 1
        ):
        ...
        if not np.isfinite(self.eog_ridge_alpha) or self.eog_ridge_alpha < 0:
...
class OfflinePreprocessingPipeline:
    ...
    def _temporal_steps(self, signal: np.ndarray) -> np.ndarray:
        """Apply record-level temporal stages in their documented order."""
        result = signal
    ...
    def transform(self, eeg: object, *, eog: object | None = None) -> OfflinePreprocessingResult:
        ...
        stages: list[str] = []
        if self.config.apply_filter:
            stages.append(f"filter:{self.config.filter_mode}")
        if self.config.detrend_order is not None:
            stages.append(f"detrend:{self.config.detrend_order}")
        values, reference_report, bad_channels = self._prepare_eeg(raw)
        stages.append(f"reference:{reference_report.method}")
        if self.config.detect_and_interpolate_bad_channels:
            stages.append("bad_channel_detection_and_interpolation")
        ...
        if self.config.use_eog_regression:
            ...
            stages.append("eog_regression:fitted_transform")
        ...
        if self.ica is not None:
            ...
            stages.append("ica:fixed_transform")
        ...
        if self.config.wavelet_config is not None:
            ...
            stages.append("wavelet_denoising")

        if not np.isfinite(values).all():
            raise RuntimeError("Offline preprocessing produced NaN or Inf")
