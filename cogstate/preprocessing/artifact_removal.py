...
class FasterReport:
    ...
    def kept_epoch_mask(self) -> np.ndarray:
        size = len(self.kept_epoch_indices) + len(self.bad_epochs)
...
def run_faster(
    ...
    *,
    ...
    values = _epoch_tensor(epochs).copy()
    if cfg.run_ica and (sample_rate is None or sample_rate <= 0):
    ...
    if cfg.run_ica:
        ...
        n_components = min(n_components, len(good_channels), len(ica_data) - 1)
        if n_components < 2:
        ...
        report.ica_converged = ica.n_iter_ < cfg.ica_max_iter
        flat_eog = None
...
class ArtifactICA:
    """ICA fitted offline and reused as a fixed streaming transform."""
    ...
    def fit(
        ...
        values = _signal_matrix(calibration_signal)
        n_components = self.config.n_components or values.shape[1]
        self._ica = FastICA(
            n_components=n_components,
        ...
        sources = self._ica.fit_transform(values)
    ...
    def transform(self, signal: np.ndarray) -> np.ndarray:
        if self._ica is None:
            raise RuntimeError("ArtifactICA is not fitted; call fit on calibration data")
        sources = self._ica.transform(_signal_matrix(signal))
    ...
    def artifact_components(self) -> tuple[int, ...]:
        return tuple(self._artifact_components)
