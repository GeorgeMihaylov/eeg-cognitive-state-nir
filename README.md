...
Новые benchmark-задачи используют явный канонический `target_id`; старые
имена target поддерживаются только предупреждающими compatibility aliases.
...
`label_q5` относится только к устройственной метрике Focus и не описывает всё
пространство когнитивных состояний. В наборе также доступны семь непрерывных
PM-целей: Attention, Engagement, Excitement, Stress, Relaxation, Interest и
Focus. Полный канонический реестр находится в
